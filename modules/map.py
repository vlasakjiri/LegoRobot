...
class Map():
    ...
    0,0 is at the top left, first y, the x.
    """

    def __init__(self):
        self.rotation = Rotation.up
        self.current_position = (3, 4)
        self.map = np.full((6, 9), dtype=Map_tile,
                           fill_value=Map_tile.not_discovered)
    ...
    def get_forward_tile_value(self):
        return self.map[self.__get_forward_tile_pos__(self.current_position,
                                                      self.rotation)]
    ...
    def write_sensor_values(self, values):
        ...
        for i in range(-1, 2, 1):
            ...
            self.map[self.__get_forward_tile_pos__(
                self.current_position, self.rotation + i)] = to_set
